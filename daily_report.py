...
def get_realtime_indices():
    """【修復版】使用 history 抓取大盤與櫃買，避免 fast_info 快取錯誤"""
...
def calculate_technical_indicators(df):
    if df.empty or len(df) < 20:
        for col in ['RSI', 'K', 'D', 'MACD', 'Signal', 'Hist', 'MA20']:
            df[col] = 50.0 if col in ['RSI', 'K', 'D'] else 0
        return df
...
def generate_market_text():
    ...
    kline_text = (
        f"• **加權指數 (大型股)**：`{twii_rt_price:,.2f}` 點 ({tw_icon} {twii_rt_pct:+.2f}%)\n"
    ...
    if foreign is not None:
        ...
        if foreign > 50:
            chip_comment = "外資大舉掃貨，熱錢湧入。"
        elif foreign < 0 and trust > 0:
            chip_comment = "投信土洋對作，內資護盤。"
        elif foreign < -50:
            chip_comment = "外資無情提款，權值股承壓。"
        else:
            chip_comment = "法人動作不大，回歸基本面。"

        inst_text = (
            f"今日三大法人合計：**{total_net:+.2f} 億元**\n"
    ...
    rsi = float(twii['RSI'].iloc[-1]) if (not twii.empty and 'RSI' in twii.columns) else 50.0
    k = float(twii['K'].iloc[-1]) if (not twii.empty and 'K' in twii.columns) else 50.0
    d = float(twii['D'].iloc[-1]) if (not twii.empty and 'D' in twii.columns) else 50.0
    macd_hist = float(twii['Hist'].iloc[-1]) if (not twii.empty and 'Hist' in twii.columns) else 0.0
    p_macd_hist = float(twii['Hist'].iloc[-2]) if (not twii.empty and 'Hist' in twii.columns and len(twii) > 1) else 0.0
    k_prev = float(twii['K'].iloc[-2]) if (not twii.empty and len(twii) > 1) else 50.0
    d_prev = float(twii['D'].iloc[-2]) if (not twii.empty and len(twii) > 1) else 50.0

    rsi_desc = "⚠️ 過熱警報 (隨時面臨修正)" if rsi > 75 else "🟢 落底反彈 (超賣區浮現買點)" if rsi < 30 else "🟡 中性震盪"

    if macd_hist > 0 and macd_hist > p_macd_hist:
        macd_trend = "紅柱擴大，多頭動能強勁"
    elif macd_hist > 0:
        macd_trend = "紅柱縮減，多方力竭"
    elif macd_hist < p_macd_hist:
        macd_trend = "綠柱縮減，空方力道衰退"
    else:
        macd_trend = "綠柱擴大，空方主導"
    ...
    tech_text = (
        f"• **RSI (14)**：`{rsi:.1f}` ｜ {rsi_desc}\n"
        f"• **KD (9,3,3)**：K `{k:.1f}` / D `{d:.1f}` ｜ {kd_desc}\n"
        f"• **MACD 動能**：{macd_trend}"
    )

    vix_trend = "下降" if c_vix_price < p_vix_price else "飆升"
    intl_text = (
        f"昨夜美股 S&P 500 **{'收紅' if c_sp_price > p_sp_price else '收黑'}** (收 {c_sp_price:,.0f} 點)。\n"
    ...
    ma20 = float(twii['MA20'].iloc[-1]) if (not twii.empty and 'MA20' in twii.columns) else twii_rt_price
    ...
    eval_text = (
        f"🎯 **大盤短線支撐**：`{support:,.0f} 點` ｜ 🎯 **上檔壓力**：`{resistance:,.0f} 點`\n"
        f"> **操盤建議**：{adv}"
    )
...
async def send_daily_report(channel):
    msg = await channel.send("📡 **正在抓取大盤、櫃買指數與三大法人籌碼...**")
...
@bot.event
async def on_ready():
    print(f'📊 大盤分析機器人 {bot.user} 已上線！(櫃買修復版)')
